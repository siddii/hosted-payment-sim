...
class OffsiteGatewaySimTest < Test::Unit::TestCase
  ...
  def load_request_fields
    request_fields =  {}
    YAML.load_file('request_fields.yml').each do |x|
      request_fields.merge!(x['key'] => x['placeholder'])
    end
  ...
  def test_post_completed
    payload = load_request_fields

    post 'execute/completed', payload
    assert last_response.ok?
